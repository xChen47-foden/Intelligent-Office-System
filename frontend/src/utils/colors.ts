export function getCssVariable(str: string, fallback?: string) {
  const value = getComputedStyle(document.documentElement).getPropertyValue(str).trim()
  return value || fallback || '#409EFF'
}
